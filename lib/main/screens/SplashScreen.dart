...
class SplashScreenState extends State<SplashScreen> {
  ...
  Future<void> init() async {
    // Safety timer: navigate after 5 seconds even if API is stuck
    Timer(Duration(seconds: 5), () {
      _navigateToNext();
    });

    await getLanguageList("").then((value) {
      if (value.themeColor != null) {
  ...
  Future<void> _navigateToNext() async {
    ...
    if (appStore.isLoggedIn && getIntAsync(USER_ID) != 0) {
      await getUserDetail(getIntAsync(USER_ID)).then((value) async {
        ...
        if (value.deliverymanVehicleHistory != null && value.deliverymanVehicleHistory!.isNotEmpty) {
        ...
        if (value.deletedAt != null) {
          ...
          if (value.app_version.isEmptyOrNull || value.app_version != packageInfo.version) {
            updateUserStatus({"id": getIntAsync(USER_ID), "app_version": packageInfo.version}).then((value) {}).catchError((e) {
              log(e);
            });
          }
        ...
        log(e);
        // If user detail fails (likely because server changed), log out and go to login
        logout(context);
      });
    } else {
            ...
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  16.height,
                  Text(language.appName == "$defaultKeyNotFoundValue(9)" ? mAppName : language.appName, style: boldTextStyle(size: 20), textAlign: TextAlign.center),
                  Spacer(),
