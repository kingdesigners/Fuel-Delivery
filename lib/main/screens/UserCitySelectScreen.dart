...
class UserCitySelectScreenState extends State<UserCitySelectScreen> {
  ...
  getCountryApiCall() async {
    ...
    await getCountryList().then((value) {
      ...
      print("Countries found: ${countryData.length}");
      
      if (countryData.isNotEmpty) {
        int savedCountryId = getIntAsync(COUNTRY_ID);
        bool hasMatch = false;
        
        for (var element in countryData) {
          if (element.id == savedCountryId) {
  ...
  getCityApiCall({String? name}) async {
    if (selectedCountry == null) {
      print("Cannot fetch cities: No country selected");
      return;
    }
    ...
    await getCityList(countryId: selectedCountry!, name: name).then((value) {
      ...
      print("Cities found for country $selectedCountry: ${cityData.length}");
      
      int savedCityId = getIntAsync(CITY_ID);
      for (var element in cityData) {
        if (element.id == savedCityId) {
          selectedCity = savedCityId;
          break;
        }
      }
      setState(() {});
                                    ...
                                    children: [
                                      Text(mData.name ?? '',
                                          style: selectedCity == mData.id
                                              ...
                                              : primaryTextStyle()),
                                      selectedCity == mData.id ? Icon(Icons.check_circle, color: ColorUtils.colorPrimary) : const SizedBox(),
                                    ],
                                  ),
                                ),
                              );
